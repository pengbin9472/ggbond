...
| 项目 | 说明 |
|------|------|
| **上游仓库** | pengbin9472/ggbond |
| **Fork 仓库** | bayma888/ggbond-bmai |
...
| 数据库凭据 | user=`ggbond`, password=`ggbond`, dbname=`ggbond` |
...
psql -U ggbond -h 127.0.0.1 -d ggbond -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER ggbond WITH PASSWORD 'ggbond';
...
```bash
# 连接数据库
psql -U ggbond -h 127.0.0.1 -d ggbond
...
# 执行 SQL 文件
psql -U ggbond -h 127.0.0.1 -d ggbond -f migration.sql
```
...
- [上游仓库](https://github.com/pengbin9472/ggbond)
