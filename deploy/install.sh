#!/bin/bash
#
# GGbond Installation Script
# GGbond 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/pengbin9472/ggbond/main/deploy/install.sh | bash
#
...
get_current_version() {
    if [ -f "$INSTALL_DIR/ggbond" ]; then
        ...
        "$INSTALL_DIR/ggbond" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="ggbond_${version_num}_${OS}_${ARCH}.tar.gz"
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/ggbond" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
