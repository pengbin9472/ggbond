...
get_latest_version() {
    ...
    # Fetch the test release info
    local release_info
    release_info=$(curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPO}/releases/tags/${RELEASE_TAG}" 2>/dev/null)
    ...
    if [ -z "$LATEST_VERSION" ]; then
        print_error "$(msg 'failed_get_version')"
        print_info "No test release assets found."
        exit 1
    fi
...
validate_version() {
    ...
    # For test script, "test" tag is always valid
