# GGbond Deployment Files

This directory contains files for deploying GGbond on Linux servers and Apple-silicon Macs.
...
| `ggbond.service` | Systemd service unit file |
| `ggbond-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/ggbond/main/deploy/docker-deploy.sh | bash

# Or download first, then run
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/ggbond/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
```bash
# Clone repository
git clone https://github.com/Wei-Shaw/ggbond.git
cd ggbond/deploy
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs ggbond | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/ggbond-datamanagement.sock`
...
# Restart GGbond only
docker compose -f docker-compose.local.yml restart ggbond
...
# View logs
docker compose logs -f ggbond

# Restart GGbond only
docker compose restart ggbond
...
| `ADMIN_EMAIL` | No | `admin@ggbond.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf ggbond-complete.tar.gz deployment/

# Transfer to new server
scp ggbond-complete.tar.gz user@new-server:/path/to/destination/
...
GGbond supports three methods to connect to Gemini:
...
```bash
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/ggbond/main/deploy/install.sh | sudo bash
```

### Manual Installation

1. Download the latest release from [GitHub Releases](https://github.com/Wei-Shaw/ggbond/releases)
2. Extract and copy the binary to `/opt/ggbond/`
3. Copy `ggbond.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/ggbond.service
   ```
...
The main config file is at `/etc/ggbond/config.yaml` (created by Setup Wizard).
...
/opt/ggbond/
├── ggbond              # Main binary
├── ggbond.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 ggbond
...
# View detailed logs
docker compose logs --tail=100 ggbond
...
# View recent logs
sudo journalctl -u ggbond -n 50

# Check config file
sudo cat /etc/ggbond/config.yaml
...
GGbond supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.ggbond.org](https://tls.ggbond.org/)** to get TLS fingerprint information for different devices and browsers.
